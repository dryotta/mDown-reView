...
For CLI/single-instance forwarded files (i.e., when a second instance is spawned with `mdownreview <path>` while a primary window is open), the receiving window's per-window scope is extended to include each forwarded file's canonical parent (asset-protocol scope + watcher allowlist) BEFORE the renderer is asked to open the new tab. See [`docs/security.md`](../security.md) rule 17 for the chokepoint discipline and [`docs/best-practices-common/tauri/v2-patterns.md`](../best-practices-common/tauri/v2-patterns.md) for the multi-window registration model; this prevents the "path not in workspace" race the prior implementation suffered.
