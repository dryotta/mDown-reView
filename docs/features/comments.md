...
- **Surface labels**: `CommentsPanel` renders a `📄 File` pill in place of `Line N` for file-anchored threads. `ViewerToolbar` shows a `CommentBadge` next to the `Comment on file` button (count = `FileBadge.file_level_count`) so users see the unresolved file-level count without opening the panel. The CLI prints `[<id>] file-level …` instead of `[<id>] line ?`.

Authoring lives in two places:

1. The `+` button in the `CommentsPanel` header opens an inline `CommentInput` whose Save submits `addComment(filePath, text, { kind: "file" })`.
2. The `Comment on file` button in `ViewerToolbar` (universally surfaced, including for binary / image / audio / "too large" / deleted-file viewers) sets `requestFileLevelInput(filePath)` so the panel auto-opens the same inline composer.
