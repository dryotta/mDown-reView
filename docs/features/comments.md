...
Anchoring survives file edits through a layered algorithm — exact match at original line, full-document exact search, normalized-stripped substring search (recovers visual-mode selections that span GFM inline markers like `**bold**`/`[link](url)`), **multi-line projection match with best-prefix recovery** (recovers cross-block selections, soft-wrapped paragraph selections, inline-HTML rendered text like `<kbd>Ctrl</kbd>`, and Unicode/whitespace-normalized text — when the full selection no longer matches, trailing words are trimmed at word boundaries until a prefix anchors), line fallback, fuzzy Levenshtein, then orphan. The algorithm is implemented in Rust core and specified in [`docs/architecture.md`](../architecture.md) §Re-anchoring algorithm. Orphaned comments never disappear silently — they surface in the `DeletedFileViewer` when their file is removed, and in an orphan banner when their anchor text no longer matches.
...
- MRSF v1.0 + v1.1 schema + re-anchoring algorithm — [`docs/architecture.md`](../architecture.md).
