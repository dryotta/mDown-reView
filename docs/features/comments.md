...
### Structured-anchor backend (not yet surfaced in UI)

The Rust backend defines typed anchor variants for structured content: `Image_rect` (percentage-coordinate region on a bitmap), `Csv_cell` (row + col + header), `Json_path` (JSONPath + optional scalar value), `Html_range` (CSS selector + offset range + selected text), `Html_element` (CSS selector + tag + preview). Each has a resolver in `src-tauri/src/core/anchors/<matcher>.rs` dispatched through `resolve_anchor`. Supporting infrastructure exists (`src/lib/html-bridge.ts` for iframe postMessage, `src/hooks/useCollisionLayout.ts` for marker overlap), but **no viewer currently wires up UI entry points to create these anchors**. All structured viewers (JSON tree, CSV table, HTML preview, Mermaid diagram, Image) support only file-level commenting today; users can switch to source view for line-level commenting on text-based formats.

When structured-anchor UI entry points are implemented, they should follow the viewer consistency tiers defined in [`viewer-consistency.md`](viewer-consistency.md). Every entry point goes through the existing `add_comment` chokepoint with the canonical `Anchor` discriminated union (rule 1 in [`docs/architecture.md`](../architecture.md)).
...
Every commentable viewer (Markdown visual mode, Source view) exposes a floating context menu on right-click. The menu carries three actions:

- **💬 Comment on selection** — commits the current text selection through the same `useSelectionToolbar` flow as the mouseup-driven toolbar; disabled when no non-empty selection exists at the click site.
- **🔗 Copy link to line** — writes a workspace-relative `mdrv://<rel>?line=<n>` URL to the clipboard via `buildCommentLink`. The `mdrv://` scheme is **copy-only** today (no OS handler is registered); the format is fixed so PR / chat consumers can parse it. When the click site does not resolve to a real source line, the action relabels to **🔗 Copy link to file** and emits `mdrv://<rel>` without `?line=`.
- **✅ Mark line as discussed** — calls `addComment` with `body="discussed"` and `severity="none"` against a `{ kind: "line", line }` anchor; this is the canonical "marker" convention for transient review notes that don't merit a full thread. Disabled when the click site has no line.

The menu is **keyboard-reachable**: `Shift+F10` and the `ContextMenu` key are routed by `useGlobalShortcuts` to `commentsSlice.activeViewerContextMenu`, a callback that the active viewer registers on mount via `useViewerContextMenu`. The shortcut computes a position from the current selection rect (else `(100, 100)`) and the registered callback resolves line + selection state itself, so the keyboard path never depends on `document.activeElement` or synthetic-event dispatch.
...
- **UI components:** `src/components/comments/{CommentInput,CommentThread,CommentsPanel,CommentBadge,LineCommentMargin,SelectionToolbar,CommentContextMenu}.tsx`; `src/components/SettingsDialog.tsx` (display-name field). `CommentsPanel` carries the file-level `+` button (Group B / iter 5) which mounts an inline `CommentInput` whose Save submits a `{ kind: "file" }` anchor through `add_comment`; the focused row (DOM focus from J/K and click) gets a `:focus-within` halo via CSS + `aria-current="true"` so the user can see which thread R will resolve. It also hosts an iter-9 filter row (search input + severity chips + workspace-wide toggle) whose state drives `useFilteredComments`; when "workspace-wide" is on, the list flattens threads from every file with sidecars via `useWorkspaceComments`. `ReactionRow` (in `CommentThread.tsx`) renders the `👍/✓/✗` glyphs that dispatch `update_comment` with an `add_reaction` patch via the VM `addReaction` action — see [`docs/architecture.md`](../architecture.md) §v1.1 reactions schema. `CommentContextMenu.tsx` renders the F6 right-click menu (three actions: comment-on-selection, copy-link, mark-discussed) and is gated by `hasSelection`/`hasLine` props so disabled actions can never silently no-op.
- **Right-click wiring:** `src/hooks/{useContextMenu,useViewerContextMenu}.ts` — `useContextMenu` is the generic open/position state primitive; `useViewerContextMenu` is the per-viewer chokepoint that owns line resolution, selection priming, action dispatch, and the `commentsSlice.activeViewerContextMenu` registration that makes Shift+F10 / ContextMenu key reach the same code path as a real right-click. `src/lib/comment-link.ts` builds the `mdrv://<rel>?line=<n>` deep-link copied to clipboard by the "Copy link to line" action — the scheme is **copy-only / no OS handler**, fixed-format for PR / chat consumers.
