...
Each viewer renders a `ViewerToolbar` overlay at the top of its scroll container. It is `position: sticky; top: 0; z-index: 2`, so it stays in view as the body scrolls. It is composed via three named slots — `leftSlot` (active-view toggle Source ↔ Visual for files that support both, plus the Wrap toggle for source views), `centerSlot` (per-viewer affordances; Markdown's TOC, Source's search, etc.), and `rightSlot` (universal actions: **Comment on file**, **Next unresolved (workspace-wide)** chord-bound to `N`, plus a **file/orphan pill** that surfaces `FileBadge.file_level_count` and orphan count without opening the panel). The pill is the only place these counts are visible in viewer chrome (rule 31 in [`docs/architecture.md`](../architecture.md) covers the producer side; UI consumption follows the same `deriveAnchor` discriminator path).
