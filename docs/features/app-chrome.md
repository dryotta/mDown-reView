...
- **Viewer toolbar (sticky):** `src/components/viewers/ViewerToolbar.tsx` · `src/styles/viewer-toolbar.css` (`position: sticky; top: 0; z-index: 2`) · slot-prop composition (the `patterns-children-over-render-props` rule)
...
The `set_theme` IPC also pushes the new preference into `AppHandle::set_theme(Option<Theme>)` after persisting so the **native title bar AND the native menu bar follow the new value live** — not only on the next cold start. On Windows this iterates every webview window and calls muda's `set_theme_for_hwnd` to re-theme each HMENU (`"system"`/`"light"`/`"dark"` map to `MenuTheme::Auto`/`Light`/`Dark`); on macOS it forwards to the runtime which flips `NSApp.appearance` app-wide via tao's `event_loop.set_theme`, covering the global menu bar and every `NSWindow` in one call. Newly-spawned windows additionally apply `commands::config::apply_persisted_theme_to_window` immediately after `WebviewWindowBuilder::build()` because `.theme()` at builder time only flips Windows DWM immersive-dark-mode — without the post-build push, a fresh window's HMENU would default to `MenuTheme::Auto` (follow OS) even when the user has an explicit override persisted, producing a dark menu on a light app whenever the OS theme differed. The **OS file picker** dialog (`@tauri-apps/plugin-dialog::open`) inherits the same NSApp appearance on macOS (so it follows the app preference automatically once `set_theme` is called) but on Windows is rendered by the OS shell via `IFileDialog` and is **not themable per-app by any documented Win32 API** — this is the same constraint VS Code, Chrome, and Office hit on Windows, and is accepted under the [Non-Goals](../principles.md) (we are a viewer, not a custom-file-picker platform).

For the canonical rule set governing per-window menus, window-scoped events, registry lifecycle, state isolation, label conventions, and the cross-window allowlist, consult the `multiwin-*` rule family in `tauri-coding-expert`'s bundled knowledge.
...
- Multi-window menu/event/state contracts — `multiwin-*` rule family.
- ViewerToolbar slot composition over prop-bag growth — `architecture-avoid-boolean-props` + `patterns-children-over-render-props` rules.
