...
24. **Window `CustomEvent` payloads with multiple variants use a tagged discriminator.** When a `mdownreview:*` (rule 15) or other window-scoped custom event represents more than one logical case, the `detail` shape is a discriminated union with a `kind: 'a' | 'b' | …` tag — never optional fields whose presence implicitly types the event. Listeners narrow on `detail.kind` with `assertNever` exhaustiveness; producers cannot drift the shape silently. Canonical: `CommentFlashDetail.kind: 'file' | 'line' | 'range' | 'unmatched'` in `src/lib/comment-flash.ts` mirrors the `Anchor` tagged enum (rule 31 in [`docs/architecture.md`](architecture.md)).
