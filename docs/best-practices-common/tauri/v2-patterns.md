...
**Applies to:** `file-changed`, `folder-changed`, `sidecar-config-changed`, `comments-changed`, `args-received`, `open-file-tab`, `excalidraw-flush-before-close`, `focus-tab`, menu events. Only `update-progress` (always to main) and truly global preference changes warrant broadcast.
