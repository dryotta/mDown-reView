...
### IPC mock hygiene
9. Every browser E2E spec mocks the eleven canonical init commands: `get_launch_args`, `read_dir`, `read_text_file`, `load_review_comments`, `save_review_comments`, `check_path_exists`, `get_log_path`, `get_file_badges`, `get_file_comments`, `scan_review_files`, `update_watched_files`. (`e2e/browser/fixtures/error-tracking.ts:93-148` — defaults applied across the per-test-mock and fallback branches.)
...
26. **IPC event payload fixtures.** Tests that exercise `listenEvent("file-changed", …)`, `listenEvent("folder-changed", …)`, `listenEvent("comments-changed", …)`, or `listenEvent("update-progress", …)` MUST construct payloads via the shared factories in `src/__tests__/fixtures/ipc-event-fixtures.ts`. Inline `{ path, kind }` / `{ file_path }` / `{ content_length, chunk_length }` object literals are forbidden (enforced by `src/__tests__/ipc-event-fixture-conformance.test.ts`); when adding a new IPC event, extend `EventPayloads` (`src/lib/tauri-events.ts`) and the fixture file in the same PR. Rationale: synthetic payloads silently mis-shaped the real watcher emission three times in a row in #298, making tests pass while production silently no-opped. How-to: section 4 in [`best-practices-project/test-patterns.md`](best-practices-project/test-patterns.md).
27. Native-E2E WebView2 host-state failures matching `0x8007139F` / `ERROR_SERVICE_NOT_ACTIVE` / `CDP HTTP did not become ready` are classified `ENVIRONMENTAL — retry recommended` by `exe-implementation-validator` ONLY when the diff has no changes under `e2e/native/` or to `src-tauri/src/lib.rs`, `src-tauri/src/main.rs`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/build.rs`, `playwright.native.config.ts`, `scripts/stage-cli.mjs`, or `src-tauri/binaries/**`. `iterate-one-issue` Step 6d.0 retries native-E2E once for this class, free of the 5-attempt forward-fix budget. (Issue #316.)
...
29. Native E2E suite uses **two-config isolation** to prevent cross-spec contamination of process-global Windows state, plus **per-spec window-scope reset** to prevent cross-spec contamination of per-window watcher state: (a) `playwright.native.config.ts` (default `npm run test:e2e:native`) spawns one shared CDP-attached debug binary via `globalSetup`; specs share it serially, listed alphabetically. (b) `playwright.installer.config.ts` (`npm run test:e2e:native:installer`) runs `installer.spec.ts` only with NO `globalSetup` — required because Playwright's `globalSetup` is config-level not per-project (https://playwright.dev/docs/api/class-testproject — `TestProject` has no `globalSetup` property). The installer spec's silent NSIS install/uninstall via `execSync` would kill any shared debug binary if co-located. (c) Inside `playwright.native.config.ts`'s shared-binary suite, the `nativePage` fixture in `e2e/native/fixtures.ts` invokes the debug-only `reset_window_scope_for_test` IPC after the `__TAURI_INTERNALS__` readiness check and before `await use(page)`, clearing `tree_watched_dirs[label]`, `watched_paths[label]`, and `extra_watched_dirs[label]` so each spec starts with an empty per-window scope precondition. Specs that need to drive an outside-file open through the production `args-received` chain (avoiding races with the renderer's deferred bootstrap drain) call the debug-only `open_file_via_test` IPC via the `openFileViaTest` fixture helper — same registration shape as `set_root_via_test`, single FILE input, no asset-scope grant. Cross-reference rule 17 in [`docs/security.md`](security.md) (which enumerates all `WindowRegistry::register` chokepoint sites including the test-only IPC). Issues #364 + #366.
