...
7. MRSF sidecar read/write/serde/reparenting lives in Rust (`src-tauri/src/core/sidecar/`, `core/comments.rs`); TypeScript never parses or serializes sidecars. Tests asserting on the on-disk shape produced by these libraries follow rule 26 in [`docs/test-strategy.md`](test-strategy.md).
