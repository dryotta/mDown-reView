...
16. Cross-slice state changesfrom a single user action group into one store action. (`store/index.ts:149-161` `closeTab`.) See rule 28 for the related no-shared-boolean-mount rule that enforces single-surface invariance for multi-surface UI intents.
