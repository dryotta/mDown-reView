...
31. `MatchedComment.matched_line_number == 0` means "no file-line representation" (`Anchor::Unknown` and unresolved `Anchor::WordRange`). `matched_line_number == 1` for `Anchor::File` is a synthetic file-anchor sentinel meaning "file-anchored, no line." Producers in `src-tauri/src/commands/comments/get.rs` populate these sentinels deliberately. Frontend filter sites that bucket comments into per-line gutter badges MUST exclude both before bucketing — the future canonical filter is `src/hooks/useThreadsByLine.ts` (consumer-side enforcement lands in a follow-up iteration; today the matcher producer-side contract is what this rule pins).
