...
### State boundaries
15. Zustand `persist` serializes only UI state: `theme`, `folderPaneWidth`, `commentsPaneVisible`, `root`, `expandedFolders`, `authorName`, `readingWidth`, `recentItems`, `tabs`, `activeTabPath`, `updateChannel`, `zoomByFiletype`. `ghostEntries`, `lastSaveByPath`, `lastFileReloadedAt`, `lastCommentsReloadedAt`, `viewModeByTab`, `fileMetaByPath`, `updateStatus`, `settingsDialogOpen`, comments, scroll values, tab back/forward history (`tabHistory` slice — session-only), and `viewerPrefsSlice.allowedRemoteImageDocs` are never persisted — trust decisions like remote-image allowance must not silently survive an app restart. The persist config carries a monotonic `version` and `migrate` fn; v1 (issue #89) strips Windows `\\?\` verbatim prefixes from `root` / `activeTabPath` / `tabs[].path` / `recentItems[].path` / `expandedFolders` keys for snapshots written by pre-#89 clients (`store/migrations/v1-strip-verbatim.ts`). (`store/index.ts` `partialize`; tabs slice `store/tabs.ts:39-58`; viewerPrefs slice `store/viewerPrefs.ts`; tabHistory slice `store/tabHistory.ts`.)
...
### Native menu
24. Native OS menu events are forwarded as `menu-*` Tauri events handled in `src/hooks/useMenuListeners.ts`, not invoked as commands. (`lib.rs:193-212`; `useMenuListeners.ts:22-54`.) **Window-frame and devtools actions are an exception** — they are handled entirely in Rust inside `on_menu_event` (no `menu-*` event, no frontend listener) because they operate on the `WebviewWindow` resolved from the menu-id label and have no UI side-effect that React needs to learn about: `win-minimize`, `win-bring-all`, `new-window`, `toggle-devtools` (latter requires the `devtools` Cargo feature on `tauri` so it works in release builds — see [`docs/features/app-chrome.md`](features/app-chrome.md)).
...
28. A single store boolean MUST NOT gate the mounting of two distinct UI surfaces. If two surfaces represent two presentations of the same intent, model the intent as a discriminated-union state so only one surface is renderable at any time. Enforced by the custom ESLint rule `local/no-shared-boolean-mount` (`eslint-rules/no-shared-boolean-mount.js`), which flags two sibling `{identifier && <Component/>}` JSX expressions referencing the same identifier under the same parent JSX element/fragment. Canonical application: `settingsDialogOpen` in `src/store/index.ts` gates a single `<SettingsView/>` dialog (issue #160 collapsed the prior `settingsSurface` union + `authorDialogOpen` child-modal into one boolean since author editing is now inline). A single boolean is compliant when it gates exactly one surface. Ternary (`flag ? <X/> : null`) and member-expression gates are out of scope of the lint rule and must be caught in review.

### Renderer chrome
29. The WebView's default OS context menu is suppressed app-wide by a single window-level `contextmenu` listener installed in `src/main.tsx` (alongside the global error handlers). **The app ships no in-app context menus** — every right-click is a hard suppression. New context-menu UX (if revisited) MUST mount its own React-rendered floating component; the WebView's native menu is never the answer. The renderer's inspector is reachable in **both debug and release builds** via **Window → Toggle Developer Tools** (`F12`) — see rule 24 and [`docs/features/app-chrome.md`](features/app-chrome.md). Regression test: `src/__tests__/globalContextMenuSuppression.test.ts`.
