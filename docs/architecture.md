...
31. `MatchedComment.matched_line_number == 0` means "no file-line representation" (`Anchor::Unknown` and unresolved `Anchor::WordRange`). `matched_line_number == 1` for `Anchor::File` is a synthetic file-anchor sentinel meaning "file-anchored, no line." Producers in `src-tauri/src/commands/comments/get.rs` populate these sentinels deliberately. Frontend filter sites that bucket comments into per-line gutter badges MUST exclude both — and `is_orphaned: true` threads — before bucketing. Canonical filter site: `src/hooks/useThreadsByLine.ts` (consumer side). Canonical producer site: `src-tauri/src/commands/comments/get.rs`. The filter routes through `deriveAnchor` (`src/lib/anchor-derive.ts`) for `kind === 'file'` discrimination — never raw `anchor_kind` string equality.
