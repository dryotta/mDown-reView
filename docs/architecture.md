...
7. MRSF sidecar read/write/serde/reparenting lives in Rust (`src-tauri/src/core/sidecar/`, `core/comments.rs`); TypeScript never parses or serializes sidecars. Tests that assert on the on-disk shape produced by the underlying YAML library (`serde-saphyr` / `saphyr`) MUST follow rule 26 in [`docs/test-strategy.md`](test-strategy.md) — fixtures are built by invoking the library's actual emitter, not by hand-writing what an author thinks the shape should be.
...
### File-size budgets
23. Any file >400 lines in `src/components/` or `src-tauri/src/` is a structural smell and must be split. Shared-chokepoint files (`src/store/index.ts`, `src/App.tsx`, `src-tauri/src/lib.rs`) get a 500-line budget. Auto-generated files (`src/lib/bindings.ts`) and test files (`src-tauri/src/**/tests.rs`) are exempt — generated code can't be split, and exhaustive coverage tables grow naturally. **Files currently exceeding their budget** (must be split before this PR's follow-up issue closes): `src-tauri/src/lib.rs` 804/500 (extract `BINDINGS_HEADER` + `build_specta_builder` + the codegen export block into `specta_export.rs`; also extract `build_window_menu` and `route_args_through_registry`); `src-tauri/src/core/comments.rs` 569/400; `src-tauri/src/commands/comments/badges.rs` 561/400; `src-tauri/src/commands/comments/mod.rs` 533/400; `src-tauri/src/core/types/wire.rs` 474/400; `src-tauri/src/core/matching.rs` 417/400 (17 lines over). All other files in `src/` and `src-tauri/src/` are under budget. Compliance is spot-checked at PR review; a periodic `wc -l` pass refreshes the over-budget list.
