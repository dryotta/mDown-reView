...
## Requirement: E2E tests cover open-folder and file navigation

The Playwright suite SHALL verify that a user can open a folder and navigate to a file.

### Scenario: Opening a folder populates the folder tree
- **WHEN** the user triggers "Open Folder" and selects a directory via dialog mock
...
- **THEN** a new tab appears with the file's name and the viewer renders the content

### Scenario: Clicking a source file opens it in the source viewer
- **WHEN** the user clicks a `.ts` file in the folder tree
- **THEN** the source viewer renders syntax-highlighted code
...
- **WHEN** the user clicks `+` on a paragraph, types a comment, and submits
- **THEN** the comment appears below the paragraph in a CommentThread

### Scenario: Comments persist across page reload
- **WHEN** a comment has been added and the file is closed and reopened
