...
- **WHEN** `openFile` is called with a file path not already open
...
- **WHEN** `addComment` is called with a filePath, anchor, and text
- **THEN** `store.commentsByFile[filePath]` contains a comment with matching anchor and text
...
The Zustand `persist` middleware SHALL serialize only tab scroll positions, workspace root, and theme preference — not comment content (comments persist via sidecar files, not localStorage).
