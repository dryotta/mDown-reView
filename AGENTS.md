...
src/
  ...
  components/
    ...
    viewers/
      ...
      MermaidView.tsx
      ExcalidrawView.tsx    ← lazy 3-mode viewer for `.excalidraw[lib]` and `.excalidraw.{png,svg}`
      ExcalidrawSourceMode.tsx ← Source-mode wrapper that runs extractScene on PNG/SVG variants
      mermaid/
      ...
      excalidraw/
        ExcalidrawBanners.tsx     ← presentational banners (FirstEntry, SaveError, Conflict) + SavedPill
        PersistentExcalidrawHost.tsx ← module-scope mount host for persistent editor instances
    comments/               ← CommentInput, CommentThread, CommentsPanel, CommentBadge, CommentMarker, SelectionToolbar
    AboutDialog.tsx
    ErrorBoundary.tsx
  lib/
    excalidraw/             ← issue #352 — extractScene, saveScene, stable-hash,
                              error-mapping (typed WorkspaceWriteError → friendly copy),
                              flush-registry (close-flush handshake), seen-flag (localStorage chokepoint)
...
src-tauri/src/
  commands/                 ← Tauri commands grouped by feature area:
    ...
    fs_write.rs             ← issue #352 workspace-write chokepoint (write_workspace_text/binary,
                              ensure_writable: NTFS-ADS reject + parent canonicalisation +
                              workspace allowlist + extension allowlist + size cap +
                              atomic commit + self-write suppression registration; arch rule 32)
    open_file_registry.rs   ← multi-window same-file singleton primitive
                              (claim_open_file, release_open_file(s), focus_window)
    close_flush.rs          ← close-flush handshake commands (CloseRequested → drain pending saves)
