...
The converged `ci.yml` (windows-x64 + windows-arm64 + macos-arm64 builds + tests + native-e2e + bundle integrity) runs on every PR to `main` automatically — no path filter at the workflow level, so docs-only PRs see the full check status (heavy jobs are correctly skipped via the `changes` job; the aggregate `Test (Linux)` gate accepts skip-on-skip). canary/release reuse this same workflow via `workflow_call` with `sign=true`.
...
- **Branch protection check name:** `Test (Linux)` (the aggregate gate inside ci.yml). All other CI jobs feed into it.
