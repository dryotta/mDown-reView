...
- `docs/test-strategy.md` (rule 28 — Test data fidelity) — fixtures consuming external library output must mirror the registered version's actual on-disk shape.

**Always check:**
- Hand-written fixture for code consuming external-library output (filenames, paths, structured data)? Flag as a Rule 28 violation in [`docs/test-strategy.md`](../../docs/test-strategy.md). Canonical incident: PR #295 prune_logs underscore/dot regression.
