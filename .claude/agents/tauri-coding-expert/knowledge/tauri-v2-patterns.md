---
tags: [tauri-v2, ipc, events, architecture]
...
| `drag-drop-rejected` | one (target window) | `emit_to(label, …)` | `commands/drag_drop.rs::handle_dropped_paths` (Empty / OverCap / AllFailed / Partial outcomes) and `lib.rs::on_window_event` `WindowEvent::DragDrop` arm (over-cap before `paths.clone()`, per `docs/performance.md` rule 1) | ✅ |
