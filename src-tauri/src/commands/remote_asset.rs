...
//! back the raw bytes + content-type. The frontend converts the bytes into a
...
/// pool across all `fetch_remote_asset` invocations.
...
#[tauri::command]
pub async fn fetch_remote_asset(url: String) -> Result<RemoteAssetResponse, String> {
    // Bound 1: https-only.
    ...
    // Bounds 2: timeouts come from the shared client.
    let response = client()
        .get(parsed)
        .send()
        .await
    ...
    // Bound 3: streamed body cap. Abort on overflow.
...
mod tests {
    ...
    async fn rejects_non_https_url() {
        for url in [
            ...
            let err = fetch_remote_asset(url.to_string())
                .await
                .expect_err(url);
            assert!(
    ...
    async fn rejects_invalid_url() {
        let err = fetch_remote_asset("not a url".into()).await.unwrap_err();
        assert!(err.contains("invalid url"), "got: {err}");
    }

    // Wiremock binds plain HTTP, so the https-only guard would reject it.
    // For the integration-shaped tests below we exercise the post-scheme
    // pipeline directly via a small helper that mirrors the command body
    // but skips the https check.
    async fn fetch_via(server_url: String) -> Result<RemoteAssetResponse, String> {
        let response = client()
            .get(&server_url)
            .send()
            .await
    ...
    async fn accepts_valid_png_returns_bytes() {
        ...
        let resp = fetch_via(format!("{}/img.png", server.uri())).await.unwrap();
    ...
    async fn accepts_content_type_with_charset() {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let resp = fetch_via(format!("{}/x.svg", server.uri())).await.unwrap();
    ...
    async fn rejects_html_content_type() {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let err = fetch_via(format!("{}/page", server.uri())).await.unwrap_err();
    ...
    async fn rejects_oversize_response() {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let err = fetch_via(format!("{}/big.png", server.uri()))
            .await
            .unwrap_err();
    ...
    async fn rejects_404_status() {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let err = fetch_via(format!("{}/missing.png", server.uri()))
            .await
            .unwrap_err();
