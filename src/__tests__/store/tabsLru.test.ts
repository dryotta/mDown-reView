...
describe("tabs slice — LRU eviction (MAX_TABS)", () => {
  it("MAX_TABS is exported and equals 5", async () => {
    expect(MAX_TABS).toBe(5);
  });

  it("evicts oldest non-active tab when opening past MAX_TABS", async () => {
    const store = useStore.getState();
    for (let i = 1; i <= MAX_TABS + 1; i++) {
      await store.openFile(`/p${i}.md`);
    }
  ...
  it("does NOT evict a recently re-activated tab", async () => {
    const store = useStore.getState();
    // Open 15 tabs
    ...
    await useStore.getState().openFile(`/p${MAX_TABS + 1}.md`);
  ...
  it("never evicts the active tab", async () => {
    const store = useStore.getState();
    await store.openFile("/p1.md");
    ...
    for (let i = 2; i <= MAX_TABS + 1; i++) {
      ...
      useStore.getState().setActiveTab("/p1.md");
      await useStore.getState().openFile(`/p${i}.md`);
    }
    const { tabs } = useStore.getState();
    expect(tabs).toHaveLength(MAX_TABS);
    expect(tabs.find((t) => t.path === "/p1.md")).toBeDefined();
  ...
  it("opening the same path twice does not duplicate and bumps lastAccessedAt", async () => {
    ...
    await useStore.getState().openFile("/p1.md");
  ...
  it("evicts viewModeByTab and lastSaveByPath entries for the evicted tab", async () => {
    const store = useStore.getState();
    await store.openFile("/p1.md");
  ...
  it("MAX_TABS overflow evicts fileMetaByPath entry for the evicted tab", async () => {
    const store = useStore.getState();
...
describe("filterStaleTabs — MAX_TABS rehydration cap", () => {
  ...
  it("retains active tab even when it has the oldest lastAccessedAt", async () => {
    const tabs: Tab[] = [];
  ...
  it("treats missing lastAccessedAt (legacy snapshot) as 0 and evicts those first", async () => {
    const tabs: Tab[] = [];
  ...
  it("does nothing when tab count <= MAX_TABS and all files exist", async () => {
    const tabs: Tab[] = [];
