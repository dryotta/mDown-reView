...
| [`audio/`](./audio/) | `AudioViewer` | Tiny WAV files (1s tone, 1s chord) |
| [`binary/`](./binary/) | `BinaryPlaceholder` | ZIP, raw binary blob, no-extension blob |
...
### Audio — `audio/`
- Two short WAV files (1s each).
- AudioViewer should show transport controls; play/pause should work.
- Zoom is intentionally disabled for audio.

### Binary — `binary/`
- `BinaryPlaceholder` shows MIME hint + size + appropriate icon.
- ZIP files get the archive icon; raw `.bin` files get the generic icon.
...
# 2 WAV files + ZIP + raw binary blobs (stdlib only)
python samples/generate_audio_and_binary.py
```
...
`generate_images.py`. Audio fixtures (sine + chord) are generated by
`generate_audio_and_binary.py`. Remote-image references point at
well-known stable hosts (GitHub avatars, raw.githubusercontent,
picsum.photos) and are loaded through mdownreview's bounded HTTPS
image proxy.
